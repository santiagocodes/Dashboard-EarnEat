   ...
   onCanjear = (premio) => {
      ...
      fetch('/api/usuarios/' + this.state.user.id + '/premios', {
         ...
         .then((res) => {
            if (res.status === 200) {
               this.setState({
                  message: `¡ Felicidades ${this.state.user.nombre} !  Puedes ver tu premio en tu área personal.`,
                  visible: true,
               });
            } else {
               this.setState({
                  message:
                     'Lo siento, pero tu premio no se ha canjeado. Verifica que tengas saldo suficiente en tu cuenta. Si tienes los puntos y el problema persiste, contacta a algun administrador.',
                  visible: true,
               });
            }
         })
         .catch((err) => {
            this.setState({
               message:
                  'Lo siento, pero tu premio no se ha completado. Verifica que tengas saldo suficiente en tu cuenta. Si tienes los puntos y el problema persiste, contacta a algun administrador.',
               visible: true,
            });
         });
   };
         ...
         <Container className="containerAll" fluid={true}>
            ...
            <Segment raised>
               ...
               <Header as="h3" textAlign="center" color="teal">
                  ¡ Enhorabuena {user.nombre}, tienes acumulados {this.props.saldo}{' '}
                  puntos !
               </Header>
               <Divider />
