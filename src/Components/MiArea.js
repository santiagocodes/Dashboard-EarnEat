...
export default class MiArea extends Component {
   ...
   onLoadPremiosCanjeados = () => {
      ...
      fetch(`/api/usuarios/${this.props.user.id}/premios_canjeados`)
         .then((res) => res.json())
         .then((premios_canjeados) =>
            this.setState({ premios_canjeados: premios_canjeados })
         );
   };
   ...
   handleClick = (evento) => {
      ...
      console.log(
         'Acabas de tratar de cambiar la contraseña, pero como enterderas no puedo permitir eso. El código esta completo, pero lo tengo desabilitado.'
      );
   ...
   onDisfrutado = (premio_canjeado) => {
      ...
      fetch(
         `/api/usuarios/${premio_canjeado.usuario_id}/premios/${premio_canjeado.id}`,
         {
            method: 'PATCH',
         ...
         .then((res) => {
            ...
            if (res.status === 200) {
               this.setState({
                  message: 'Tu premio sera marcado como utilizado.',
                  visible: true,
               });
               this.onLoadPremiosCanjeados();
            } else
               this.setState({
                  message:
                     'Ha occurido un error. Vuelve a tratar más tarde o contacta a un administrador.',
                  visible: true,
               });
         })
         .catch((err) => {
   ...
   render() {
      ...
      const premios_canjeados = this.state.premios_canjeados.filter(
         (premio) => premio.utilizado === 0
      );
      const premios_canjeados_utilizados = this.state.premios_canjeados.filter(
         (premio) => premio.utilizado === 1
      );
      return (
                           ...
                           <Popup
                              trigger={
                                 <Button
                                    ...
                                    circular={true}
                                    onClick={() =>
                                       this.onDisfrutado(premio_canjeado)
                                    }
                                 >
                                    ¿Disfrutado?
                                 </Button>
